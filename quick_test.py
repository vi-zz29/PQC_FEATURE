...
def preprocess_real(img: np.ndarray) -> tuple:
    ...
    # COMBINE
    real_edges = cv2.bitwise_or(
        internal,
        outer
    )

    # REMOVE ONLY TINY NOISE
...
def main():
    ...
    if len(sys.argv) >= 5 and len(sys.argv) % 2 == 0:
        ...
        cad_path = next(
            (p for p in cad_candidates if Path(p).exists()),
            None
        )

        real_path = next(
            (p for p in real_candidates if Path(p).exists()),
            None
        )
        ...
        if cad_path is None or real_path is None:
            ...
            print(
                "Multi: python quick_test.py real.png cad1.png 'Object 1' cad2.png 'Object 2'"
            )
